...
export function getLoadingTimeout(pathname: string, isMobile: boolean = false): number {
  const routeTiming = LOADING_CONFIG.ROUTE_TIMING[pathname as keyof typeof LOADING_CONFIG.ROUTE_TIMING];
  const baseTimeout = routeTiming || LOADING_CONFIG.TIMING.NORMAL;
  
  return isMobile 
    ? Math.min(baseTimeout, LOADING_CONFIG.PERFORMANCE.MOBILE_TIMEOUT)
    : baseTimeout;
}
...
export class LoadingPerformanceMonitor {
  ...
  logMetrics(pathname: string) {
    if (process.env.NODE_ENV === 'development') {
      console.log(`Loading time for ${pathname}: ${this.getDuration().toFixed(2)}ms`);
    }
  }
}
