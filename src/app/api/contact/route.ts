...
function validateInput(name: string, email: string, subject: string, message: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  const suspiciousPatterns = [
    ...
    /document\./i, /window\./i, /alert\(/i, /prompt\(/i
  ];
...
function checkRateLimit(ip: string): { allowed: boolean; remaining: number; resetTime: number } {
  ...
  if (current.count >= RATE_LIMIT.maxRequests) {
    ...
    if (now < current.resetTime + RATE_LIMIT.blockDurationMs) {
      return { allowed: false, remaining: 0, resetTime: current.resetTime + RATE_LIMIT.blockDurationMs };
    }
...
export async function POST(request: Request) {
  try {
    ...
    // Validate inputs
    const validation = validateInput(sanitizedName, sanitizedEmail, sanitizedSubject, sanitizedMessage);
    if (!validation.valid) {
      return NextResponse.json(
        { 
          success: false, 
